...
drop.addEventListener("drop",function(e){e.preventDefault();drop.classList.remove("on");var dt=e.dataTransfer,entries=[];if(dt.items&&dt.items.length&&dt.items[0].webkitGetAsEntry){for(var i=0;i<dt.items.length;i++){var en=dt.items[i].webkitGetAsEntry&&dt.items[i].webkitGetAsEntry();if(en)entries.push(en)}}if(entries.length){toast("正在读取文件夹…");walkEntries(entries).then(function(fs){if(fs.length)uploadFiles(fs);else toast("没读到文件（空文件夹？）")})}else{uploadFiles(dt.files)}});
// 拖文件夹：递归读出里面所有文件（含子文件夹），再统一上传
function walkEntries(entries){return new Promise(function(resolve){var files=[],remaining=entries.length;if(!remaining)return resolve(files);function decr(){if(--remaining<=0)resolve(files)}function traverse(entry,done){if(!entry){done();return}if(entry.isFile){entry.file(function(f){files.push(f);done()},done)}else if(entry.isDirectory){var rd=entry.createReader(),kids=[];(function readMore(){rd.readEntries(function(ents){if(!ents.length){if(!kids.length)return done();var left=kids.length;kids.forEach(function(k){traverse(k,function(){if(--left<=0)done()})})}else{kids=kids.concat([].slice.call(ents));readMore()}},done)})()}else done()}entries.forEach(function(en){traverse(en,decr)})})}
...
function uploadFiles(files){files=Array.prototype.slice.call(files||[]);if(!files.length)return;navTo({view:"upload"});var pc=$("progress");pc.classList.remove("hide");pc.innerHTML="";var doCompress=$("cmp").checked,albumId=(NAV.album!=null&&NAV.type==null)?NAV.album:null,done=0,fail=0,canceled=0;
  ...
  function maybeHide(){if(!pc.querySelector(".pitem")){releaseWake();pc.classList.add("hide")}}

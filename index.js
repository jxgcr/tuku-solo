...
const DEFAULT_BYTE_LIMIT = 10522669875;   // 默认容量上限 9.8G，压在 R2 免费额度(10GB)内、永不扣费
const FREE_CEIL = 10 * GB;                // R2 免费额度天花板；超过这个数，Cloudflare 按量计费
...
// 实际生效上限：客户在设置里「自助开阀」调过的值(存 config)优先，否则用部署默认(BYTE_LIMIT)。
// 这是「安全阀」：默认卡在免费额度内永不扣费；客户知情后可自己调大，超出部分自负 CF 费用。
async function effectiveLimit(env) {
  const o = Number(await getConf(env, "byte_limit"));
  return Number.isFinite(o) && o > 0 ? o : byteLimit(env);
}
...
async function storeUpload(env, file, albumId, thumb) {
  ...
  const lim = await effectiveLimit(env);
...
async function handleMe(env) {
  ...
  const used = await usedBytesOf(env);
  const lim = await effectiveLimit(env);
  return json({ count, usedBytes: used, byteLimit: lim, usedGB: fmtGB(used), limitGB: fmtGB(lim), freeCeil: FREE_CEIL, overFree: lim > FREE_CEIL });
}
// 客户自助「开阀」：调整自己的容量上限。低于 10G=免费安全区；高于 10G=知情后自负 CF 费用。
async function handleSetLimit(request, env) {
  const body = await request.json().catch(() => ({}));
  const gb = Number(body.gb);
  if (!Number.isFinite(gb) || gb < 1) return json({ error: "请输入有效容量（GB，至少 1）" }, 400);
  if (gb > 2000) return json({ error: "太大了，请输入 2000 GB 以内" }, 400);
  const bytes = Math.round(gb * GB);
  await setConf(env, "byte_limit", String(bytes));
  return json({ ok: true, byteLimit: bytes, limitGB: fmtGB(bytes), overFree: bytes > FREE_CEIL });
}
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (request.method === "POST" && path === "/api/limit") return await handleSetLimit(request, env);
...
function openDetail(im){var b=$("dBody");b.innerHTML="";
  ...
  if(im.kind==="image"){var ex=document.createElement("div");ex.style.marginTop="12px";ex.innerHTML="<div style='font-weight:600;margin-bottom:6px'>📷 拍摄信息（EXIF）</div><div class='muted' id='exifBody'>读取中…</div>";b.appendChild(ex);loadExif(im)}
  show("detailOverlay");
}
/* ---------- EXIF 读取（纯浏览器解析 JPEG APP1，无外部库） ---------- */
function exifRange(n,fn){var a=[];for(var k=0;k<n;k++)a.push(fn(k));return a}
function readIFD(dv,tiff,ifd,le){var res={};if(ifd<0||ifd+2>dv.byteLength)return res;var n=dv.getUint16(ifd,le);for(var i=0;i<n;i++){var e=ifd+2+i*12;if(e+12>dv.byteLength)break;var tag=dv.getUint16(e,le),type=dv.getUint16(e+2,le),count=dv.getUint32(e+4,le),vo=e+8;var us=({1:1,2:1,3:2,4:4,5:8,7:1,9:4,10:8})[type]||1;var sz=us*count;var off=sz>4?tiff+dv.getUint32(vo,le):vo;if(off<0||off>dv.byteLength)continue;if(type===2){var s="";for(var j=0;j<count&&off+j<dv.byteLength;j++){var c=dv.getUint8(off+j);if(c===0)break;s+=String.fromCharCode(c)}res[tag]=s.replace(/^\s+|\s+$/g,"")}else if(type===3){res[tag]=count>1?exifRange(count,function(k){return dv.getUint16(off+k*2,le)}):dv.getUint16(off,le)}else if(type===4){res[tag]=count>1?exifRange(count,function(k){return dv.getUint32(off+k*4,le)}):dv.getUint32(off,le)}else if(type===5){var f=function(k){var a=dv.getUint32(off+k*8,le),b2=dv.getUint32(off+k*8+4,le);return b2?a/b2:0};res[tag]=count>1?exifRange(count,f):f(0)}else if(type===10){var g=function(k){var a=dv.getInt32(off+k*8,le),b3=dv.getInt32(off+k*8+4,le);return b3?a/b3:0};res[tag]=count>1?exifRange(count,g):g(0)}}return res}
function parseExif(buf){var dv=new DataView(buf);if(dv.byteLength<4||dv.getUint16(0)!==0xFFD8)return null;var o=2;while(o+4<dv.byteLength){var m=dv.getUint16(o);if((m&0xFF00)!==0xFF00)break;var len=dv.getUint16(o+2);if(m===0xFFE1&&dv.getUint32(o+4)===0x45786966){var tiff=o+10;var le=dv.getUint16(tiff)===0x4949;var ifd0=tiff+dv.getUint32(tiff+4,le);var t0=readIFD(dv,tiff,ifd0,le);var out={};if(t0[0x010F])out.make=t0[0x010F];if(t0[0x0110])out.model=t0[0x0110];if(t0[0x0132])out.datetime=t0[0x0132];if(t0[0x8769]){var te=readIFD(dv,tiff,tiff+t0[0x8769],le);if(te[0x9003])out.datetime=te[0x9003];if(te[0x829D])out.fnumber=te[0x829D];if(te[0x8827])out.iso=te[0x8827];if(te[0x920A])out.focal=te[0x920A];if(te[0x829A])out.exposure=te[0x829A]}if(t0[0x8825]){var gp=readIFD(dv,tiff,tiff+t0[0x8825],le);var la=gp[0x0002],lo=gp[0x0004];if(la&&lo&&la.length===3&&lo.length===3){var lat=la[0]+la[1]/60+la[2]/3600;if(gp[0x0001]==="S")lat=-lat;var lng=lo[0]+lo[1]/60+lo[2]/3600;if(gp[0x0003]==="W")lng=-lng;if(isFinite(lat)&&isFinite(lng))out.gps={lat:lat,lng:lng}}}return out}o+=2+len}return null}
function loadExif(im){fetch(im.link,{headers:{Range:"bytes=0-262143"}}).then(function(r){return r.arrayBuffer()}).then(function(buf){var el=$("exifBody");if(!el)return;var x=null;try{x=parseExif(buf)}catch(e){}if(!x||(!x.make&&!x.model&&!x.datetime&&!x.gps&&!x.fnumber&&!x.focal)){el.innerHTML="无拍摄信息（原图不含，或上传时勾选压缩被去除）";return}var rows=[];var dev=[x.make,x.model].filter(Boolean).join(" ");if(dev)rows.push(["设备",dev]);if(x.datetime)rows.push(["拍摄时间",String(x.datetime).replace(/^(\d{4}):(\d{2}):(\d{2})/,"$1-$2-$3")]);var ps=[];if(x.fnumber)ps.push("f/"+(Math.round(x.fnumber*10)/10));if(x.exposure)ps.push(x.exposure<1?("1/"+Math.round(1/x.exposure)+"s"):(x.exposure+"s"));if(x.iso)ps.push("ISO"+x.iso);if(x.focal)ps.push(Math.round(x.focal)+"mm");if(ps.length)rows.push(["参数",ps.join(" · ")]);var html=rows.map(function(r){return "<div class='info' style='border-bottom:1px solid var(--line)'><span class='il'>"+r[0]+"</span><span>"+esc(r[1])+"</span></div>"}).join("");if(x.gps){var u="https://uri.amap.com/marker?position="+x.gps.lng+","+x.gps.lat;html+="<div class='info' style='border-bottom:1px solid var(--line)'><span class='il'>位置</span><span><a href='"+u+"' target='_blank' rel='noopener' style='color:#a78bfa'>"+x.gps.lat.toFixed(6)+", "+x.gps.lng.toFixed(6)+" 🗺</a></span></div>"}el.innerHTML=html}).catch(function(){var el=$("exifBody");if(el)el.innerHTML="读取失败"})}
...
function openSettings(){var b=$("setBody");b.innerHTML="<div class='muted' style='padding:10px 0'>加载中…</div>";closeDrawer();show("setOverlay");
  api("/api/me").then(function(d){
    ...
    var badge=d.overFree?"<span style='color:var(--amber)'>（已超免费额度·会计费）</span>":"<span style='color:var(--ok)'>（免费额度内·不扣费）</span>";
    row("容量上限","<b>"+gbNow+" GB</b> "+badge+" <button class='sm' id='btnLimit' style='margin-left:6px'>调整</button>");
...
function openLimitEditor(d){var b=$("setBody");b.innerHTML="";
  ...
  box.innerHTML="<div style='font-weight:600;font-size:1rem;margin-bottom:10px'>调整容量上限（存储自由）</div>"+
    "<div class='muted' style='line-height:1.7;margin-bottom:14px'>默认 <b>9.8GB</b> 是安全线——在 Cloudflare <b style='color:var(--ok)'>免费额度内、永不扣费</b>。<br>你可以自己调大以获得更多空间，但 <b style='color:var(--amber)'>超过 10GB 的部分，Cloudflare 会按用量从你自己的卡扣费</b>，费用由你承担。要不要放开，你说了算。</div>"+
    "<div style='display:flex;gap:8px;align-items:center;margin-bottom:6px'><input id='limGB' type='number' step='0.1' min='1' value='"+(d.byteLimit/1073741824).toFixed(1)+"' style='width:130px'><span class='muted'>GB</span></div>"+
    "<div class='muted' style='font-size:.8rem'>建议想免费就填 9.8；要更多就填你愿意付费的上限。</div>"+
    "<div style='display:flex;gap:8px;margin-top:16px'><button class='sm' id='limCancel'>返回</button><button class='sm pri' id='limSave'>确认调整</button></div>";
  b.appendChild(box);
  $("limCancel").onclick=openSettings;
...
function uploadFiles(files){files=Array.prototype.slice.call(files||[]);if(!files.length)return;navTo({view:"upload"});var pc=$("progress");pc.classList.remove("hide");pc.innerHTML="";var doCompress=$("cmp").checked,albumId=(NAV.album!=null&&NAV.type==null)?NAV.album:null,done=0,fail=0,canceled=0;
  ...
  function failButtons(f,item,acts){acts.appendChild(mkbtn("🔄 重试","pbtn",function(){attempt(f,item).then(function(){reloadFiles()}).catch(function(){})}));acts.appendChild(mkbtn("🗑 移除","pbtn del",function(){item.remove()}))}
  function attempt(f,item){
    item.className="pitem";var bar=item.querySelector("i"),pct=item.querySelector(".pct"),acts=item.querySelector(".pacts");bar.style.width="0%";pct.textContent="0%";acts.innerHTML="";
    var ctrl={canceled:false,paused:false,abort:null};
    ...
    }).then(function(){item.classList.add("done");pct.textContent="完成";acts.innerHTML=""}).catch(function(e){
      if(ctrl.canceled||/已取消/.test(e.message||"")){item.classList.add("canceled");pct.textContent="已取消";acts.innerHTML="";failButtons(f,item,acts);throw{canceled:true}}
      item.classList.add("err");pct.textContent=(e&&e.message)||"上传失败";acts.innerHTML="";failButtons(f,item,acts);throw{failed:true};
    });
  }
  function runOne(i){if(i>=files.length){toast("完成 "+done+" 个"+(fail?("，失败 "+fail+"（可在下方逐个重试）"):"")+(canceled?("，取消 "+canceled):""));reloadFiles();setTimeout(function(){if(!fail&&!canceled)pc.classList.add("hide")},1600);return}
    var f=files[i];var item=document.createElement("div");item.className="pitem";item.innerHTML="<div class='pn'><span>"+esc(f.name)+"</span><span class='pct'>0%</span></div><div class='pbar'><i></i></div><div class='pacts'></div>";pc.appendChild(item);
    attempt(f,item).then(function(){done++}).catch(function(x){if(x&&x.canceled)canceled++;else fail++}).then(function(){runOne(i+1)});
  }
  toast("上传中…");runOne(0);
}
